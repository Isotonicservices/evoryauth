  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden">
      ...
      <main className="flex-1 p-8 overflow-y-auto max-w-5xl z-10 animate-fadeIn">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-red-400 font-mono text-xs gap-4">
            <div className="h-8 w-8 border-2 border-red-500 border-t-transparent rounded-full animate-spin shadow-[0_0_15px_rgba(239,68,68,0.5)]"></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 stagger-children">
            <div className="lg:col-span-2 flex flex-col gap-8">
              {/* Profile Info */}
              <GlowCard glowColor="red" className="relative overflow-hidden group">
                {/* Decorative background element */}
                <div className="absolute right-0 top-0 w-64 h-64 bg-red-500/5 rounded-full blur-[80px] -mr-32 -mt-32 pointer-events-none" />
                
                <h3 className="text-sm font-bold mb-6 flex items-center gap-2 text-white relative z-10">
                  <User className="h-4 w-4 text-red-400" /> Identity Matrix
                </h3>
                
                <div className="flex flex-col sm:flex-row gap-8 items-start relative z-10">
                  {/* Avatar */}
                  <div className="relative group shrink-0">
                    <div className="absolute inset-0 bg-red-500 rounded-2xl blur-xl opacity-20 group-hover:opacity-40 transition-opacity" />
                    <div className="h-24 w-24 rounded-2xl bg-gradient-to-br from-red-600 to-red-800 p-[3px] relative z-10">
                    ...
                    <div className="absolute -bottom-2 -right-2 p-1.5 bg-red-500 rounded-lg border-2 border-[#020106] z-20">
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
                    ...
                    <div className="flex flex-col gap-2 md:col-span-2">
                      ...
                      <div className="bg-black/40 border border-red-500/20 px-4 py-3 rounded-xl font-semibold text-red-400 shadow-inner flex items-center justify-between">
              ...
              {/* API access tokens */}
              <GlowCard glowColor="red" className="relative overflow-hidden">
                <div className="absolute right-0 bottom-0 w-64 h-64 bg-red-500/5 rounded-full blur-[80px] -mr-32 -mb-32 pointer-events-none" />
                
                <div className="flex justify-between items-start mb-6 relative z-10">
                  <div>
                    <h3 className="text-sm font-bold flex items-center gap-2 text-white">
                      <KeyRound className="h-4 w-4 text-red-400" /> Global API Access Token
                    </h3>
                    <p className="text-[11px] text-slate-400 mt-1 max-w-md">
                      Authenticate external pipelines, CLI tools, or server-to-server infrastructure.
                    </p>
                  </div>
                  <button onClick={handleRegenerateKey} disabled={regenerating} className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg text-xs font-bold border border-red-500/30 transition-all">
                    <RefreshCw className={`h-3 w-3 ${regenerating ? "animate-spin" : ""}`} /> Roll Key
                  </button>
                </div>

                <div className="relative group z-10">
                  <div className="absolute inset-0 bg-gradient-to-r from-red-500/20 to-red-500/20 rounded-xl blur opacity-20 group-hover:opacity-40 transition-opacity" />
                  <div className="bg-black/80 border border-white/10 p-4 rounded-xl flex items-center gap-4 relative">
                    <code className="font-mono text-sm text-red-300 break-all select-all flex-1">
                    ...
                    >
                      {copiedKey ? <CheckCircle2 className="h-5 w-5 text-red-400" /> : <Copy className="h-5 w-5 text-slate-400" />}
                    </button>
                  </div>
                </div>
              </GlowCard>
